...
> **本机（DSH Desktop / Windows）环境适配，先读 [`DSH-LOCAL.md`](./DSH-LOCAL.md)：** 依赖已装齐（playwright + Chromium、
> fonttools、pywin32）；视觉 audit 走已实测可用的 PowerPoint/WPS COM（不会触发"没有渲染器"分支）；首次 CJK 转换要下载约 40 MB 字体（约 75 秒）。
> 本技能目录是 `%APPDATA%\dsh-desktop\harness\skills\html-to-pptx\`，即下文 `<skill_dir>`。
